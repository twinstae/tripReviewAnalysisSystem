...
class MapView(generic.ListView):
    ...
    def get_queryset(self):  # 컨텍스트 오버라이딩
        ...
        attractions_list = Attraction.objects.all().order_by('-star_rating')[:6]
...
def distance_r(selected_attractions, pk, n = 15):
    ...
    candidate_df = pd.DataFrame(np.zeros(shape = (n,3)), columns = ['rating', 'end_pk', 'dist'])
